...
void APlayerPawn::AddInventory (AInventory *item)
{
		...
		if(keyScore && (NETWORK_GetState( ) == NETSTATE_SERVER ) ){
					...
					SERVERCOMMANDS_DoSDSound(2, ULONG( player - players ) , SVCF_ONLYTHISCLIENT);
					//SERVERCOMMANDS_SetSectorSecretFound( LONG( sector - sectors ) );

					SERVERCOMMANDS_PrintMid(levelscore, player - players, SVCF_ONLYTHISCLIENT);
					//SERVERCOMMANDS_DoSDSound(2, ULONG( player - players ), SVCF_ONLYTHISCLIENT);
...
bool PLAYER_Responder( event_t *pEvent )
{
	...
	if ( pEvent->type == EV_GUI_KeyDown )
	{
 		...
 		if ( pEvent->data2 == ' ' )
		{
			...
			if (( teamplay || ( teamgame && TemporaryTeamStarts.Size( ) == 0 ) || teamlms || teampossession || teamcoop ) && (( dmflags2 & DF2_NO_TEAM_SELECT ) == false ))
			{
	...
	if ( CLIENT_PREDICT_IsPredicting( ) == false )
	{
		...
		if ( NETWORK_GetState( ) != NETSTATE_CLIENT )
		{
			...
			if (( level.time & 31 ) == 0 && ( player->Powers & PW_REGENERATION ) && ( player->health ))
			{
				...
				{
					S_Sound( player->mo, CHAN_ITEM, "ghk/qregen", 1, ATTN_NORM );
					...
					{
						SERVERCOMMANDS_SetPlayerHealth( player - players );
						SERVERCOMMANDS_SoundActor( player->mo, CHAN_ITEM, "ghk/qregen", 127, ATTN_NORM );
					}
				}
			}
		}
