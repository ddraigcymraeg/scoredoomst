...
void P_PlayerInSpecialSector (player_t *player)
{
			...
			if ( sector->special & SECRET_MASK )
			{
				...
				//{
					...
					if( teamcoop || cooperative)
						player->lPointCount+=250;
					if(teamcoop&&player->bOnTeam)
						TEAM_SetScore( player->ulTeam, TEAM_GetScore(player->ulTeam ) + player->lPointCount, true );
				...
				//}
					//we want the other clients to know this has been
					//triggered already so they cant score.
				if ( (NETWORK_GetState( ) == NETSTATE_SERVER) && ( teamcoop || cooperative) ){
					SERVERCOMMANDS_SetPlayerPoints( player - players );
					SERVERCOMMANDS_SetPlayerKillCount( ULONG( player - players ) , MAXPLAYERS, 0, 3 );

					// Also, update the scoreboard.
					SERVERCONSOLE_UpdatePlayerInfo( ULONG( player - players ), UDF_FRAGS );
					SERVERCONSOLE_UpdateScoreboard( );
					SERVERCOMMANDS_CheckPointLimit(   player );
					SERVERCOMMANDS_DoSDSound(1, ULONG( player - players ), SVCF_ONLYTHISCLIENT);
					//SERVERCOMMANDS_SetSectorSecretFound( LONG( sector - sectors ) );
					SERVERCOMMANDS_PrintMid("Secret Found! +250 Bonus!", player - players, SVCF_ONLYTHISCLIENT);
	...
	//ghk:  [BC] Sector specials are server-side.
	if ( NETWORK_GetState( ) == NETSTATE_CLIENT )
	{
...
//	int i;
	...
	if ( NETWORK_GetState( ) != NETSTATE_CLIENT )
	{
		...
		{
			if ( level.totaltime >= (int)(timelimit * TICRATE * 60) && ( GAME_GetEndLevelDelay( ) == 0 ))
			{
					...
					if ( DUEL_GetState( ) == DS_INDUEL )
					{
						...
						if (( lDueler1 != -1 ) && ( lDueler2 != -1 ))
						{
							if (( players[lDueler1].fragcount ) == ( players[lDueler2].fragcount ))
							{
								...
								if ( level.totaltime == (int)(timelimit * TICRATE * 60 ))
								{
									if ( NETWORK_GetState( ) == NETSTATE_SERVER )
				...
				else
				{
					...
					//GAME_SetEndLevelDelay( 1 * TICRATE ); //GHK
					...
					for ( ulIdx = 0; ulIdx < MAXPLAYERS; ulIdx++ )
					{
						...
						else if ( players[ulIdx].lPointCount == lHighestFrags )
							bTied = true;
					}
					...
					{
						//g_SDLimitHit=1;
						//SERVERCOMMANDS_SetSDLimitHit(1); //GHK dont support timelimit for now
