...
//GHK
//const char *sd_serveraddonpack_md5="\0";
//const char *sd_clientaddonpack_md5="\0";
//const char *sd_addonpack_md5="\0";
...
CVAR (Bool, sd_first_run, true, CVAR_ARCHIVE); //GHK Used for prompting user to restart SDST to use add-on pack
...
//ghk
int sd_finale_nextmap_state=0;
int didonce = false; //for finale & intermission sounds
//ghk
bool sd_do_default_monsters = false;
//int sd_do_wad_epi_restart=0;
...
CVAR (Flag, sv_bruiserrush,		dmflags2, DF2_BRUISER_RUSH);
CVAR (Flag, sv_cyberrush,		dmflags2, DF2_CYBER_RUSH);
CVAR (Flag, sv_spiderrush,		dmflags2, DF2_SPIDER_RUSH);
CVAR (Flag, sv_coopwarpto,		dmflags2, DF2_COOP_WARPTO);
CVAR (Flag, sv_aggressivebosses,dmflags2, DF2_AGGRESSIVE_BOSSES);
CVAR (Flag, sv_noaltfire,		dmflags2, DF2_NO_ALTFIRE);
...
CVAR (Flag, compat_forceblockmonster, compatflags, COMPATF_FORCEBLOCKMONSTER);
...
void D_DoomMain (void)
{
	...
	//GHK: scoredoommp.wad
	wad = BaseFileSearch( "scoredoomst.wad", NULL, true );
	if ( wad == NULL )
		I_FatalError( "Cannot find scoredoomst.wad" );
	else
		D_AddFile( wad, false );
...
//	atterm(GL_ReleaseShaders); // FIXME: change this to a more general cleanup function to avoid too many atterm handlers
	...
	for (i = 0; (arg = Wads.GetWadName (i)) != NULL; ++i)
	{
		...
		if(strcmp(arg, "SDST-ADDONPACK.WAD")==0||strcmp(arg, "sdst-addonpack.wad")==0){
	...
	*/
	//check for multiple SDST-ADDONPACKs
	//if(strlen(addonpackstr.c_str())>32)
		//addonpackstr="11111111111111111111111111111111";
/* //ghk
	if(*(addonpackstr.c_str())==0)
		addonpackstr="00000000000000000000000000000000";

	if ( NETWORK_GetState( ) == NETSTATE_SERVER ){
		sd_serveraddonpack_md5 = addonpackstr.c_str();

	}else{

		sd_clientaddonpack_md5 = addonpackstr.c_str();
	}
	*/
	if(sd_first_run){
		//I_SDMessageBox(); //ghk, not anymore for now
	...
	if ( Args.CheckParm( "-teamcoop" )) //GHK
	{
		teamcoop = 1;
	...
	GameConfig->GetSDUserReplacements(); //ghk
