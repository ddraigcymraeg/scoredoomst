	...
	if (!actor->IsKindOf (RUNTIME_CLASS (APlayerPawn)))	// [GRB]
	{
		...
		else
		{
			...
			//now just get any associated health and artifacts which should also spawn
				const PClass *newtypeinfo=NULL;
				const PClass *newtypeinfo2=NULL;
				//GHK DONT DROP EXTRA STUFF ON BOSSRUSH LEVELS
				if(PClass::FindClass ("SDSmartBomb")&&!(level.flags & (LEVEL_BRUISERSPECIAL|LEVEL_CYBORGSPECIAL|LEVEL_SPIDERSPECIAL))){
					newtypeinfo=PClass::GetRandomMonster (actor->GetClass(),0,0,0,0, true);
					
					if(newtypeinfo)
						P_DropItem (actor, newtypeinfo,0, 255); //P_DropItem (actor, newtypeinfo,1, 255);

					newtypeinfo2=PClass::GetRandomMonster (actor->GetClass(),0,0,0,0, false);
					if(newtypeinfo2)
						P_DropItem (actor, newtypeinfo2,0, 255); //P_DropItem (actor, newtypeinfo2,1, 255);
