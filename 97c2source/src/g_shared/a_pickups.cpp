...
//			ASTAR_ClearPath( toucher->player - players );
	...
	if (flags & MF_COUNTITEM)
	{
		...
		if (( cooperative || teamcoop )&& ( NETWORK_GetState( ) != NETSTATE_CLIENT )&& toucher->player != NULL)
		{
			toucher->player->itemcount++;

			//GHK add +5 to the player's score.
			toucher->player->lPointCount+=10; //was 5
			if(teamcoop&&toucher->player->bOnTeam)
					TEAM_SetScore( toucher->player->ulTeam, TEAM_GetScore(toucher->player->ulTeam ) + toucher->player->lPointCount, true );


			if (NETWORK_GetState( ) == NETSTATE_SERVER)
			{
				SERVERCOMMANDS_SetPlayerPoints( ULONG( toucher->player - players ) );
				SERVERCOMMANDS_SetPlayerKillCount( ULONG( toucher->player - players ) , MAXPLAYERS, 0, 2 );
				SERVERCONSOLE_UpdatePlayerInfo( ULONG( toucher->player - players ), UDF_FRAGS );
				SERVERCONSOLE_UpdateScoreboard( );
				SERVERCOMMANDS_CheckPointLimit(  toucher->player );
				char levelscore[10];
				sprintf(levelscore,"+ %d\n",10); //was 5
				SERVERCOMMANDS_Print(levelscore,PRINT_MEDIUM, toucher->player - players, SVCF_ONLYTHISCLIENT);
...
{
	S_SoundID (toucher, CHAN_AUTO, PickupSound, 1,
