...
CVAR (Bool,		freelook,		true,	CVAR_GLOBALCONFIG|CVAR_ARCHIVE)		// ghk Always mlook?
...
// GHK Set a flag to reset lPointCount, level.totaltime when pointlimit, timelimit hit or finale or final maplist map
static	ULONG	g_SDLimitHit = 0;
...
static void ChangeSpy (bool forward)
{
	...
	if (( demoplayback == false ) &&
		...
		((( deathmatch == false ) && ( teamgame == false ) && ( teamcoop == false )) == false ) &&
		( sv_cheats == false ))
	{
		return;
	}
...
//							LASTMANSTANDING_SetStartNextMatchOnLevelLoad( true );
				...
				else if ( possession || teampossession || cooperative || teamcoop ) //ghk for coopearative & teamcoop too
				{
					bool	bLimitHit = false;

					if ( pointlimit > 0 )
					{
						if ( possession || cooperative)
						{
							ULONG	ulIdx;
						...
						if ( bLimitHit )
						{
							if ( NETWORK_GetState( ) == NETSTATE_SERVER ){
								SERVER_Printf( PRINT_HIGH, "Winlimit hit.\n" );
								//g_SDLimitHit=1; //ghk
								SERVERCOMMANDS_SetSDLimitHit(g_SDLimitHit);
...
*/
	...
	if(teamcoop&&bPlayerStarts){ //ghk have teamgame set also?
		Val.Bool = true;
		teamgame.ForceSet( Val, CVAR_Bool );
	}
...
//*****************************************************************************
//
void GAME_SetSDLimitHit( ULONG SDFlag )
{
	g_SDLimitHit = SDFlag;
