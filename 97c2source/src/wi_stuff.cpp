...
void WI_End ()
{
	...
	/* ghk, done in doleveload, cleaner
	if(( NETWORK_GetState( ) == NETSTATE_SERVER )&&(((sv_maprotation||sv_randommaprotation)&&MAPROTATION_GetNumUnusedEntries()==0)||(GAME_GetSDLimitHit()))){
			...
			if ( NETWORK_GetState( ) == NETSTATE_SERVER ){
				SERVERCOMMANDS_SetSDLimitHit(0); //ghk reset the limit hit flag
...
void WI_UpdateCampaignStats( void )
{
	...
	if ( cp_state == 2 )
	{
		...
		else if (( teamgame || teampossession || teamcoop ) && ( players[consoleplayer].bOnTeam ))
		{
	...
	else if (cp_state == 6)
	{
		...
		if ( cnt_NumPlayers >= (( teamplay || teamlms || teamgame || teampossession || teamcoop ) ? 2 : SERVER_CalcNumPlayers( )))
		{
			cnt_NumPlayers = (( teamplay || teamlms || teamgame || teampossession | teamcoop ) ? 2 : SERVER_CalcNumPlayers( ));
			S_Sound( CHAN_VOICE, NEXTSTAGE, 1, ATTN_NONE );
			cp_state++;
		}
	}
...
void WI_DrawCampaignStats (void)
{
	...
	if (cp_state >= 6)
	{
		...
		if ( cp_state >= 7 )
		{
			if ( teamplay || teamlms || teamgame || teampossession || teamcoop )
			{
...
void WI_Ticker(void)
{
	...
	if (bcnt == 1)
	{
		...
		//ghk
	if(!didonce){ //real finale
		if ((strncmp (wminfo.next, "enDSeQ", 6) == 0)||(GAME_GetSDLimitHit())){
			
				ULONG rank = SCOREBOARD_CalcRank(consoleplayer);
				if(cooperative && !SCOREBOARD_IsTied() ){ //&& !( bIsTied )){ 
...
void WI_Start (wbstartstruct_t *wbstartstruct)
{
	...
	if(( NETWORK_GetState( ) == NETSTATE_SERVER )){
			for ( ulIdx2 = 0; ulIdx2 < MAXPLAYERS; ulIdx2++ )
		{
			if ( playeringame[ulIdx2] == false )
				continue;
			
			//ghk is all this needed??? probably not, can remove
			SERVERCOMMANDS_SetPlayerPoints( ulIdx2, me, SVCF_ONLYTHISCLIENT );


			SERVERCOMMANDS_SetPlayerKillCount( ulIdx2, me, SVCF_ONLYTHISCLIENT, 1 );
			//GHK: ALSO BARRELS ETC...
			SERVERCOMMANDS_SetPlayerKillCount( ulIdx2, me, SVCF_ONLYTHISCLIENT, 2 );

			SERVERCOMMANDS_SetPlayerKillCount( ulIdx2, me, SVCF_ONLYTHISCLIENT, 3 );

			SERVERCOMMANDS_SetPlayerKillCount( ulIdx2, me, SVCF_ONLYTHISCLIENT, 4 );
