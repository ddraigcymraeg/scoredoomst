...
// --- mobj.flags4 ---
	...
	//GHK SD Flags (and flags derived from zdoom/gzdoom)
	SDFL_FORCERADIUSDMG = 0x00000001,
...
public:
	...
	static AActor *StaticSpawn (const PClass *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool doSDReplace=true ); //ghk
...
inline AActor *Spawn (const PClass *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool doSDReplace = true)
{
	return AActor::StaticSpawn (type, x, y, z, allowreplacement, doSDReplace);
}

AActor *Spawn (const char *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool doSDReplace = true);

template<class T>
inline T *Spawn (fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool doSDReplace = true)
{
	return static_cast<T *>(AActor::StaticSpawn (RUNTIME_CLASS(T), x, y, z, allowreplacement,doSDReplace));
}
