...
static void SetTexture (int *texture, DWORD *blend, char *name8) //ghk was short
{
	char name[9];
...
static void SetTextureNoErr (int *texture, DWORD *color, char *name8, bool *validcolor) //ghk was short
{
	char name[9];
	strncpy (name, name8, 8);
	name[8] = 0;
...
void P_FreeLevelData ()
{
	...
	level.total_monsters = level.total_items = level.total_secrets =
		level.killed_monsters = level.found_items = level.found_secrets = level.killed_barrels =
		...
		level.nkeyused=level.m3keyused=level.m6keyused=level.levelexited=0;
//ghk: also make sure the finale/intermission sound global flag is reset
	didonce=false;
//
	FBehavior::StaticUnloadModules ();
	if (vertexes != NULL)
...
void P_SetupLevel (char *lumpname, int position)
{
   ...
   //ghk
	char levelname[9];
	strncpy (levelname, lumpname, 9);

	if(GameConfig->GetSDUserMapDefaultMonsters(levelname))
...
//				players[i].bSpectating = true;
		...
		if ( teamgame )
		{
			...
			if(teamcoop){ //ghk
				G_CooperativeSpawnPlayer( i, false ); //ghk second argument true?
			}else{
