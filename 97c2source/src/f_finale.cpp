...
void F_Ticker ()
{
	...
	// ghk: If we're the server, count down the amount of time left in intermission. If
	...
	if ( NETWORK_GetState( ) == NETSTATE_SERVER )
	{
		...
		{
			//SERVERCOMMANDS_PrintMid( "15 *TICRATE!!");
			g_lStopWatch = 0;
			interrupt = 1;
			//if(wminfo.next[0]==0){ //true finale?
				//char	szCommand[64];
				//sprintf( szCommand, "changemap %s", sd_global_finale_nextmap );
				//AddCommandString( szCommand );
			//}else{
				gameaction = ga_worlddone; //map20 doom2 etc..., go to next level
			//}
