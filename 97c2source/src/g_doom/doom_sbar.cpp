...
private:
	...
	void DoCommonInit ()
	{
		...
		else if ( teamgame || possession || teampossession || teamcoop )
	...
	void DrawMainBar ()
	{
		...
		if ( possession || teampossession || teamgame || teamcoop || cooperative )
		{
...
//---------------------------------------------------------------------------
	...
	void DrawFullScreenStuff ()
	{
		...
		// [BC] Draw points in possession. //ghk also coop modes
	...
	void DrawFullScreenStuffST( void )
	{
		...
		ulCurYPos = 4;
		if ( deathmatch || teamgame || teamcoop || cooperative )
		{
			if ( possession || teampossession || teamcoop || cooperative )
		...
		//}
		// Otherwise, draw the keys.    //ghk draw them, since Im only supporting co-op
		//else
		//{
			ulCurYPos = 16; //ghk
			lKeyCount = 0;
		...
		// Next, draw rank/spread in deathmatch.
		if ( deathmatch || cooperative || teamcoop ) //ghk teamcoop too?
		{
			ulCurXPos = 4;
			if ( bScale )
...
/*
	sprintf( szString, "chain count: \\cC%d/%d",CPlayer->lastkilltime,CPlayer->multicount);
	//if (players[consoleplayer].lastkilltime > 0)
			...
			//sprintf( szString, "chain count: \\cC%d", players[consoleplayer].multicount);
			V_ColorizeString( szString );
	...
	//}
		...
		// the ST/CTF score.
		else if ( skulltag || ctf || oneflagctf )
		{
			if ( CPlayer->bOnTeam )
			{
