...
extern int arrArtifacts[1000]; //ghk
CVAR (Bool, sd_defaultmonstermaps_replace_all,true,CVAR_SERVERINFO|CVAR_ARCHIVE) //ghk
...
static int sp_random_custom_monsters=1; //ghk: another dummy variable
//ghk more hardcoded vars
static int sd_artifact_chance=8;//11;
static int sd_extrahealth_chance=14; //25; //ghk
static int sd_rep_explosivebarrel_chance=25;
static int sd_rep_burningbarrel_chance=25;
static int sd_rep_arachnotron_chance=60;
static int sd_rep_archvile_chance=60;
static int sd_rep_baronofhell_chance=100;
static int sd_rep_hellknight_chance=60;
static int sd_rep_cacodemon_chance=60;
static int sd_rep_cyberdemon_chance=100;
static int sd_rep_demon_chance=60;
static int sd_rep_spectre_chance=60;
static int sd_rep_chaingunguy_chance=60;
static int sd_rep_doomimp_chance=60;
static int sd_rep_fatso_chance=60;
static int sd_rep_lostsoul_chance=60;
static int sd_rep_painelemental_chance=60;
static int sd_rep_revenant_chance=60;
static int sd_rep_shotgunguy_chance=60;
static int sd_rep_spidermastermind_chance=100;
static int sd_rep_wolfensteinss_chance=60;
static int sd_rep_zombieman_chance=60;
...
void AActor::Serialize (FArchive &arc)
{
	...
	arc << effects
		...
		<< (DWORD &)ulInvasionSpawnerID <<SDParentPClass<<origx<<origy<<origz; //ghk;
...
//
	if (mo->z > mo->floorz && !(mo->flags & MF_NOGRAVITY))
	{
		...
		if (!mo->waterlevel || mo->flags & MF_CORPSE || (mo->player &&
			!(mo->player->cmd.ucmd.forwardmove | mo->player->cmd.ucmd.sidemove)))
		{ //ghk: quarter gravity is now 3rd gravity 20.84->27.31
			mo->momz -= (fixed_t)(level.gravity * mo->Sector->gravity *
				(( mo->ulSTFlags & STFL_QUARTERGRAVITY ) ? 27.31 : (mo->flags2 & MF2_LOGRAV ? 10.24 : 81.92)));
		}
		if (mo->waterlevel > 1)
		{
...
*/
	...
	if(true && (NETWORK_GetState( ) != NETSTATE_CLIENT) ){
		...
		if(player!=NULL&&multidmgbuffer>0){
			...
			if( NETWORK_GetState( ) == NETSTATE_SERVER ){
				...
				SERVERCOMMANDS_Print(levelscore,PRINT_MEDIUM, player - players, SVCF_ONLYTHISCLIENT);
		...
		lastpnumdmg=MAXPLAYERS+1; //reset
}
...
//	if(blGetMonsters&&FBehavior::StaticModules.Size()>0){
	...
	*/
	...
	//}
		...
		DWORD ghkflags = ((AActor*)(type->Defaults))->flags;
		int ghkhealth =((AActor*)(type->Defaults))->health;

	 	if(((ghkflags & MF_COUNTKILL) && !(ghkflags & MF_FRIENDLY))){ //ghk basically Countsaskill()
			...
			if(true){ //ghk: *cough* hack
				const PClass *newtypeinfoA=NULL; // = PClass::FindClass("SDArtiSpawner");//GHK this
				int rnd = 0;
				int rndH = 0;
				...
				if(sd_extrahealth_chance>0&&sdExtraHealthRepCount>0){

					if(sd_extrahealth_chance>=rndH){ //default 35
							blGetHealth=true;
						}
					...
					if(blGetHealth){
						...
						//AActor::StaticSpawn (newtypeinfoA, ix, iy, iz+ 8 * FRACUNIT, ALLOW_REPLACE);
						//if ( NETWORK_GetState( ) == NETSTATE_CLIENT )
							//return newtypeinfo;

						//AActor	*pActor;
						//pActor=Spawn(newtypeinfoA, ix, iy, iz+ 8 * FRACUNIT, ALLOW_REPLACE);
				...
				if(blGetMonsters) //get extra health pickup flag
					return newtypeinfoA;

				if(sd_artifact_chance>0&&sdArtiRepCount>0){

					if(sd_artifact_chance>=rnd){
						...
						//AActor	*pActor2;
						//pActor2=Spawn(newtypeinfoA, ix, iy, iz+ 8 * FRACUNIT, ALLOW_REPLACE);

						//if(pActor2){
									// Since the actor successfully spawned, tell clients.
						//	if ( NETWORK_GetState( ) == NETSTATE_SERVER )
								//SERVERCOMMANDS_SpawnThing( pActor2 );
						//}
...
AActor *AActor::StaticSpawn (const PClass *type, fixed_t ix, fixed_t iy, fixed_t iz, replace_t allowreplacement, bool doSDReplace) //ghk
{
	g_lSpawnCount++;
	clock( g_SpawnCycles );
	int spwnid=0; //ghk
	...
	if (gameinfo.gametype == GAME_Doom){
		...
		if(spwnid==125)
		 ...
		 //ghk
			int sd_do_defaultmonsters_replaceall = sd_do_default_monsters&&sd_defaultmonstermaps_replace_all;

			if(doSDReplace&&((spwnid>=1&&spwnid<=10)||(spwnid>=19&&spwnid<=20)||(spwnid>=110&&spwnid<=116)||(spwnid==125)||(spwnid==149))){
				...
				switch(spwnid){
					...
					case 125: //exp barrel
						...
						if(sd_rep_explosivebarrel_chance>0&&sdExplosiveBarrelRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_explosivebarrel_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
					...
					case 149: //burning barrel
						...
						if(sd_rep_burningbarrel_chance>0&&sdBurningBarrelRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_burningbarrel_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
					...
					case 111: //archvile
						...
						if(sd_rep_archvile_chance>0&&sdArchvileRepCount>0&&!sd_do_defaultmonsters_replaceall){

							if(sd_rep_archvile_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
					...
					case 6: //arachnotron
						...
						if(sd_rep_arachnotron_chance>0&&sdArachnotronRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_arachnotron_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
							if(!sp_random_custom_monsters){
						...
						break;
					case 3: //BARONOFHELL
						//Lets See if we are Boss-Rushing?...requires addonpack
						if((((level.flags & LEVEL_BRUISERSPECIAL)&&PClass::FindClass("SDBossChainEnder"))||(dmflags2 & DF2_BRUISER_RUSH))&&!sd_do_default_monsters){ //&&!sd_do_defaultmonsters_replaceall){
							if(!sp_random_custom_monsters)
								rnd=M_Random2()%100 + 1;
							else
								rnd=pr_gkspawnchance(100) + 1;
							...
							if(sd_rep_baronofhell_chance>0&&sdBaronOfHellRepCount>0&&!sd_do_default_monsters){ //&&!sd_do_defaultmonsters_replaceall){

								if(sd_rep_baronofhell_chance>=rnd){
									//ok, now find which creature gets picked.
									//within the range
								if(!sp_random_custom_monsters){
					...
					case 113: //HellKnight
						...
						if(sd_rep_hellknight_chance>0&&sdHellKnightRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_hellknight_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
							if(!sp_random_custom_monsters){
					...
					case 19: //Cacodemon
						...
						if(sd_rep_cacodemon_chance>0&&sdCacodemonRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_cacodemon_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
							if(!sp_random_custom_monsters){
						...
						break;
					case 114: //Cyberdemon
						if((((level.flags & LEVEL_CYBORGSPECIAL)&&PClass::FindClass("SDBossChainEnder"))||(dmflags2 & DF2_CYBER_RUSH))&&!sd_do_defaultmonsters_replaceall){
							if(!sp_random_custom_monsters)
								rnd=M_Random2()%100 + 1;
							else
								rnd=pr_gkspawnchance(100) + 1;
							...
							if(sd_rep_cyberdemon_chance>0&&sdCyberdemonRepCount>0&&!sd_do_defaultmonsters_replaceall){

								if(sd_rep_cyberdemon_chance>=rnd){
									//ok, now find which creature gets picked.
									if(!sp_random_custom_monsters){
					...
					case 8: //Demon
						...
						if(sd_rep_demon_chance>0&&sdDemonRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_demon_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
									if(sdDemonRange>=255){
					...
					case 9: //Spectre
						...
						if(sd_rep_spectre_chance>0&&sdSpectreRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_spectre_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
				...
				case 2: //ChaingunGuy
						...
						if(sd_rep_chaingunguy_chance>0&&sdChaingunGuyRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_chaingunguy_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
					...
					case 5: //DoomImp
						...
						if(sd_rep_doomimp_chance>0&&sdDoomImpRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_doomimp_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
					...
					case 112: //Fatso
						...
						if(sd_rep_fatso_chance>0&&sdFatsoRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_fatso_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
					...
					case 110: //LostSoul
						...
						if(sd_rep_lostsoul_chance>0&&sdLostSoulRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_lostsoul_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
					...
					case 115: //PainElemental
						...
						if(sd_rep_painelemental_chance>0&&sdPainElementalRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_painelemental_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
							if(!sp_random_custom_monsters){
					...
					case 20: //Revenant
						...
						if(sd_rep_revenant_chance>0&&sdRevenantRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_revenant_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
							if(!sp_random_custom_monsters){
				...
				case 1: //ShotgunGuy
						...
						if(sd_rep_shotgunguy_chance>0&&sdShotgunGuyRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_shotgunguy_chance>=rnd){
								//ok, now find which creature gets picked.
							if(!sp_random_custom_monsters){
				...
				case 7: //SpiderMastermind
					...
					if((((level.flags & LEVEL_SPIDERSPECIAL)&&PClass::FindClass("SDBossChainEnder"))||(dmflags2 & DF2_SPIDER_RUSH))&&!sd_do_defaultmonsters_replaceall){
							if(!sp_random_custom_monsters)
								rnd=M_Random2()%100 + 1;
							else
								rnd=pr_gkspawnchance(100) + 1;
						...
						if(sd_rep_spidermastermind_chance>0&&sdSpiderMastermindRepCount>0&&!sd_do_defaultmonsters_replaceall){

							if(sd_rep_spidermastermind_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
							if(!sp_random_custom_monsters){
				...
				case 116: //WolfensteinSS
						...
						if(sd_rep_wolfensteinss_chance>0&&sdWolfensteinSSRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_wolfensteinss_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
							if(!sp_random_custom_monsters){
					...
					case 4: //ZombieMan
						...
						if(sd_rep_zombieman_chance>0&&sdZombieManRepCount>0&&!sd_do_default_monsters){

							if(sd_rep_zombieman_chance>=rnd){
								//ok, now find which creature gets picked.
								//within the range
								if(!sp_random_custom_monsters){
				...
				//if(PClass::FindClass ("SDSmartBomb"))
	...
	if (gameinfo.gametype == GAME_Doom){
		...
		//GHK, MAKE BOSSES more agressive
		//boss rush chain spawns are given spawnid=155 in add-on pack wad.
		//Make sure this is compatible with MP synching
		//should be
		...
		*/
		...
		if((i_compatflags & COMPATF_FORCEBLOCKMONSTER) && (actor->flags3 & MF3_ISMONSTER))
			actor->flags3 &=~MF3_NOBLOCKMONST;

		if((level.flags & (LEVEL_BRUISERSPECIAL|LEVEL_CYBORGSPECIAL|LEVEL_SPIDERSPECIAL))&&PClass::FindClass("SDBossChainEnder")&&isMonster){
	...
	{
		//if(spwnid==0||(spwnid>=1&&spwnid<=10)||(spwnid>=19&&spwnid<=20)||(spwnid>=110&&spwnid<=116)){
		//ghk debug
			if (debugfile) fprintf (debugfile, "monstertype: %s\n",type->TypeName.GetChars());
...
AActor *Spawn (const char *type, fixed_t x, fixed_t y, fixed_t z, replace_t allowreplacement, bool doSDReplace)
{
	...
	return AActor::StaticSpawn (cls, x, y, z, allowreplacement, doSDReplace);
}
...
bool P_CheckMissileSpawn (AActor* th, bool isSDBossRush)
{
	...
	if (!P_TryMove (th, th->x, th->y, false))
	{
		...
		if (BlockingMobj == NULL || !(th->flags2 & MF2_RIP))
		{
			...
			// GHK: If this is a monster spawned by A_CustomMissile subtract it from the counter.
			...
			if (BlockingLine != NULL && BlockingLine->special == Line_Horizon)
			{
				if(!isSDBossRush){ //GHK
					if ( NETWORK_GetState( ) == NETSTATE_SERVER )
						SERVERCOMMANDS_DestroyThing( th );
...
bool AActor::IsTeammate (AActor *other)
{
	...
	if ( teamgame || teamplay || teamlms || teampossession || teamcoop )
	{
