...
<body style="background:black;color: rgb(211, 211, 211);">
<center>
<?php
...
$db_host = 'localhost';
		...
		echo '<table style="width: 600px;" class="hiscoreTable"><tbody><tr>
...
// sending query
$result = mysql_query("SELECT * FROM {$table} WHERE playername='".$playername."' AND wadhiscore>0 ORDER BY WAD_EPI_DESC ");
if (!$result) {
...
// sending query
	...
	$result = mysql_query("SELECT * FROM `wad_main_level` WHERE levelhiscore > 0 AND playername = '".$playername."' ORDER BY `gmt_timestamp`");
...
while($row = mysql_fetch_assoc($result))
{
   ...
   echo "<tr style='background-color:".$bgcolor."';>";
	...
	echo "<td class='lvlslink'><a href='hardcore/wad_main_level.php?wadhash=".$row['wadhash']."&epi=".$row['episode']."&wadtitle=".urlencode($row['wad_epi_desc'])."&levelnamesid=".$row['levelnamesid']."' title='Click here to see the ".$row['wad_epi_desc']." level hi scores'>".$row['wad_epi_desc'].$dl_url."</a></td>";
	echo "<td class='smalltext'>".$row['wad_config']."</td><td>".$row['levels']."</td><td class='hiscore'>".$wadhiscore."</td><td class='player'>".$row['playername']."</td><td class='smalltext'>".gmdate("M d Y H:i:s",$row['gmt_timestamp'])."</td>";
