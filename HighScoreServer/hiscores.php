...
//V3:upx4i,upx4g,4g, upxr4f, r4f, upxr4d, r4d, upxr4c, r4c,upxr4, r4, upxr3c,r3c,upxr3b,r3b, upxr3,r3,upxr2d,r2d upxr2c,r2c,upxr2b, r2b,upxr2,r2,r1
//ONLY 4Hb
$GLOBAL_HASH=array("9b68cfd828feb49ec85b7e8fc2aeb054","ffb26320eecc240cabc1e161c28b00c3","b6881f9e7bea753a3a582a059b5f5248","e072d3ef3740879f2fd8521d43c32ac2");
//,"46b0d3828298cb280bae1aa607a8243e","91e5a1d416553e8239c0e2c7956232c2","dc02ff27299962c05dd523c3fcd199fc","efc90838c0ce26a0543fb13ceb89fe0e","90c44c2e9e7759c05ce38f9c0cf8ba65","27c7d4ec009b94b0bba43d486c756248","399a6c9790b7faf1432910593ba083a9","8bbf93da5a4a143d3001a6bd0d20173e","6123fc7c49e301b33280aed5e473eaa7","17bb09d97c8cd46822701139893fd188","ccbc47e1ab1a0f82da2c26a6b5b24243","d44f0175b25222c86f4f9e012abaf5e8","97717d58225b6268e0b189d45ab8824a","4c603f576158f49c99c6a18c93055dda","8e4b35cd86703bad994ba5df12ab49aa","be7cbff899b2e7c2cacb6f3802d1b755","2e8fbed905ae23b00c4095160df4d74c","b4f1a28f0b5e6de63b0a20ea7178dfb3","9a29370df14d36cd64a1c90c32bb88c4","df66d1539cd3cac59b0c4367b86c9a99","06576aeff2015aef5558d2174967884c","126bdf97691ac0dd2ada23211ca8779e","fcd08b5faaaa1c736b737e285b868197","15dca68b57aa5b6f36906c790532cde5","67223c5a6109aa8cd9c8d947a69cad48","ca8e2587c736771fb308640605dfe99d");

$GLOBAL_GLDEFSHASH=array("406bd65a989be42399a15cced8994ff6","9c827515e8f1e65f9274f9f8e6e645e6","0665243a60b03e10c1a282a0a67ce266","a1d8258ad652b3ca5e1fb9c9e1ec5f08");
...
$pwd=$_POST["pwd"];
$globalhash=$_POST["globalhash"];
$wadhash=$_POST["wadhash"];
$levelname=$_POST["levelname"];
$wadepiname=$_POST["wadepiname"];
$patches=$_POST["patchinfo"];
$sdgldefshash=$_POST["sdgldefshash"];
$wadhiscoreset=0;
$levelhiscoreset=0;
...
$addonpackhash=$_POST["addonpackhash"];
$configvalueshash=$_POST["configvalueshash"];
$isfinale=0; //used for properly displaying gold text for finale screen, sort of a hack.

if((isset($_POST["isfinale"])&&$_POST["isfinale"]==1)){
...
if(isset($_POST["noinfighting"])&&$_POST["noinfighting"]){
	$noinfighting=true;
...
//used for directing to pistol start dbs
if(isset($_POST["pistolstart"])&&$_POST["pistolstart"]){
	$psStr="ps_";
}else{
	$psStr="";
}
...
if((isset($_POST["mode"])&&$_POST["mode"]>1)&&$ALLOW_HARDCORE==1){
	//hardcore database!! :-)

	$noinfighting?$dbname="ddraig_".$psStr."sdhiscoresnih":$dbname="ddraig_".$psStr."sdhiscoresh";


}else{
	//wimps database :-(
	$noinfighting?$dbname="ddraig_".$psStr."sdhiscoresni":$dbname="ddraig_".$psStr."sdhiscores";
...
//ghk hack, for  PC_CP.wad, that uses mapinfo episodes, do a redirect to a new wadhash, based on map number.
if($wadhash=="1a98319938bcefd8699c62915cce5ae1"){
	if((int)substr($levelname,-2)>0&&(int)substr($levelname,-2)<33){
		$wadhash="d0d7923c01f946f9f3897e27e483e025";
	}elseif((int)substr($levelname,-2)>32&&(int)substr($levelname,-2)<41){
		$wadhash="6d0f51106c44c640aed7ee5222cee300";

	}else{
		echo "returnval:1:Invalid map name for PC_CP.wad\n:";
		exit;
...
function shuntPlayers($dbcnx,$dbname,$row,$shunttype,$hiscoretype,$udepi,$wadhash,$addonpackhash,$levelname){
	...
	if($hiscoretype==1){
		...
		if($shunttype==1){
			...
			if($udepi!=""){
				if($row['wadhiscore2']>0) //only shunt silver position, if there is a valid score to shunt
				...
				if($row['wadhiscore']>0) //only shunt gold position, if there is a valid score to shunt
		...
		if($shunttype==1){
			...
			if($udepi!=""){
				if($row['levelhiscore2']>0) //only shunt silver position, if there is a valid score to shunt
				...
				if($row['levelhiscore']>0) //only shunt gold position, if there is a valid score to shunt
...
//check that the globalhash is ok
	...
	if(array_search($globalhash, $GLOBAL_HASH)===FALSE){
		//echo "returnval:1:".$globalhash.":";

		echo "returnval:1:SCOREDOOM UPDATED TO 3.0r3H\nYour version of scoredoom is not supported by the hi score server\nPlease grab the latest version from www.scoredoom.com\n:";
		exit;
	}
...
//check that sdgldefs hash is ok, if provided
if($sdgldefshash!=0&&array_search($sdgldefshash, $GLOBAL_GLDEFSHASH)===FALSE){
	echo "returnval:1:You loaded an invalid version of the scoredoom sdgldefs.pk3 file\nPlease grab the latest version from www.scoredoom.com\n:";
	exit;
}
...
//ghk hack, to fix  zpacksd.wad!
if($wadhash!="b953314e7a3476f29785ac197c4ddbf7"){
...
if(isset($_POST["wadhiscore"])){




	//setting
	$wadhiscore=$_POST["wadhiscore"];
	$levelhiscore=$_POST["levelhiscore"];
		...
		if (!mysql_num_rows($result)) {
			...
			while ( $row = mysql_fetch_array($result) ) {
						...
						if((int)$wadhiscore > (int)$row["wadhiscore"]){
							...
							//failed to beat the wad/epi hi score
							//but make sure we are not double posting the same score, which can happen
							if(!(strcasecmp ($playername,$row['playername'])==0&&(int)$wadhiscore == (int)$row["wadhiscore"])){
		...
		//what about save games???*
			...
			while ( $row = mysql_fetch_array($result) ) {
					...
					if((int)$levelhiscore > (int)$row["levelhiscore"]){
						...
						//but make sure we are not double posting the same score, which can happen
						if(!(strcasecmp ($playername,$row['playername'])==0&&(int)$levelhiscore == (int)$row["levelhiscore"])){
