...
if(isset($_GET["wadtitle"])&&$_GET["wadtitle"]!=""){
	...
	echo "<a href='http://www.scoredoom.com'><img height='30px;' border='0' src='images/scoredoom.gif'></a><br><h2>'".$_GET["wadtitle"]."' Level Hi Score Table</h2></p>";
}else{
	echo "<a href='http://www.scoredoom.com'><img height='30px;' border='0' src='images/scoredoom.gif'></a><br><h2>ScoreDoom Official Wad Hi Score Table<h2></p>";
}
...
while($row = mysql_fetch_assoc($result))
{
   ...
   echo "<tr style='background-color:".$bgcolor."';>";
	...
	echo "<td>".$row['levelname'].$full_level_name."</td><td class='hiscore'>".$levelhiscore."<br><span style='color:silver' class='smalltext'>".$levelhiscore2."</span><br><span style='color:#A67D3D' class='smalltext'>".$levelhiscore3."</span></td><td class='player'><a href='profile.php?playername=".urlencode($row['playername'])."' target=new title='Click here to see ".$row['playername']."&#39;s profile.'>".$row['playername']."</a><br><a class='smalltext' href='profile.php?playername=".urlencode($row['playername2'])."' target=new title='Click here to see ".$row['playername2']."&#39;s profile.'>".$row['playername2']."</a><br><a class='smalltext' href='profile.php?playername=".urlencode($row['playername3'])."' target=new title='Click here to see ".$row['playername3']."&#39;s profile.'>".$row['playername3']."</a></td><td class='smalltext'>".gmdate("M d Y H:i:s",$row['gmt_timestamp'])."</td>";
...
<a class="back" href="#" onClick="history.go(-1); return false;">back</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a class="back" href="wad_main.php">view wad hi score table</a>
</center>
